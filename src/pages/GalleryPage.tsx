    ...
    <div className="min-h-screen bg-sand-light pt-24 pb-20 text-charcoal">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredItems.map((item) => (
            <div
              ...
              className="group relative rounded-3xl overflow-hidden shadow-luxury border border-gold/20 h-72 cursor-pointer bg-ocean"
            >
              <img
                src={item.image}
                alt={item.title}
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-ocean-dark/95 via-ocean-dark/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-between p-6 text-white">
                ...
                <div>
                  ...
                  <p className="text-xs text-sand/80 mt-1">{item.caption}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      {activeLightbox && (
        <div
          ...
          className="fixed inset-0 z-50 bg-ocean-dark/95 backdrop-blur-xl flex items-center justify-center p-4 sm:p-8 animate-fade-in"
        >
          <button
            onClick={() => setActiveLightbox(null)}
            className="absolute top-6 right-6 w-12 h-12 rounded-full bg-white/10 text-white hover:bg-gold hover:text-ocean-dark flex items-center justify-center transition-colors"
          >
            <X className="w-6 h-6" />
          </button>
          ...
          <div
            ...
            className="max-w-4xl w-full bg-ocean border-2 border-gold/40 rounded-3xl overflow-hidden shadow-2xl"
          >
            <img
            ...
            />
            <div className="p-6 bg-ocean-dark text-sand">
              ...
              <p className="text-xs text-sand/80 mt-2 font-sans">{activeLightbox.caption}</p>
            </div>
          </div>
        </div>
      )}
